...
todos_valores = ""
ponto_usado = False  # Flag para controlar se o ponto já foi utilizado
...
def entrada_valores(parametro):
    ...
    if parametro == '.':
        if not ponto_usado and todos_valores and todos_valores[-1].isdigit():
            todos_valores += str(parametro)
            ponto_usado = True
    elif parametro == '<--':  # Remover o último caractere
...
janela.bind('<Key>', digito_digitado)  # Vincula a função digito_digitado ao evento de pressionar teclas
